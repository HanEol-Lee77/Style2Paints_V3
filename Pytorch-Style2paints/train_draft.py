...
####################################### Stage1 (draft) train model** #######################################################
from unet import unet_model_draft
from unet import unet_model_draft
####################################### Stage1 (draft) train model** #######################################################
...
#######################################==============================#############################################
from dataset_multi import ClothDataSet_draft

from tensorboardX import SummaryWriter
writer = SummaryWriter()

# In[ ]:
#################=================== paths-stage1에서는 필요없음 ===================#################
...
args = easydict.EasyDict({
    'epochs' : 100, # 100으로 되어있었는데, yumi data set에 대해서.. 200으로 설정해둠
    'batch_size' : 8,
    'train_path' : '/home/ubuntu/data/processed/yumi/train/color',#'/data4/wangpengxiao/danbooru2017/original/train',
    'val_path' : '/home/ubuntu/data/processed/yumi/val/color',#'/data4/wangpengxiao/danbooru2017/original/val',
    'sketch_path' : '/home/ubuntu/data/processed/yumi/train/sketch',#"/data4/wangpengxiao/danbooru2017/original_sketch",
...
Unet = unet_model_draft.UNet(in_channels=4, out_channels=3) # input Sketch 1channel + hint 3channel # UNET 모델 구조를 가져옴
D = unet_model_draft.Discriminator(in_channels=3, out_channels=1)

# 텐서보드에 Unet관련 그래프 넣기
writer.add_graph(Unet, (Variable(torch.randn(1,2,4,256,256), requires_grad=True)[0]))

# 가능한 cuda.device (GPU)는 1개X
...
def val_net(args, val_loader, Unet, D, epoch, save_epoch, last_count_val):
    ...
    for i, (input, gt) in enumerate(val_loader):
        if i >= int(VAL_NUM):
            break
        input=input.float()
        # df=df.float()
...
#         gt_var = gt_var.cuda()
        ...
        save_pic(save_epoch, i, input_var, gt_var, fake_pic)
...
def tb_view_pic(input, gt, fake_pic):
