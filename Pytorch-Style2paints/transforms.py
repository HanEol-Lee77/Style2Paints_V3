...
class GroupRandomCrop(object):
    ...
    def __call__(self, img_group):
        h, w, _ = img_group.shape
        hs = self._size
        ws = self._size

        top = np.random.randint(0, h - hs)
        left = np.random.randint(0, w - ws)

        return img_group[top:hs+top, left:ws+left]
...
class GroupScale(object):
    ...
    def __call__(self, img_group):
        if img_group.shape[2] == 3:
            return cv2.resize(img_group, self._size, cv2.INTER_LINEAR)
        else: 
            return resize_mv(img_group, self._size, cv2.INTER_LINEAR, img_group.shape[2])
...
def resize_mv(img, shape, interpolation, n_c):
    return np.stack([cv2.resize(img[..., i], shape, interpolation)
                     for i in range(n_c)], axis=2)
